...
function loadAirplaneData(countries, amount, update = false) {
	...
	jq.get(dataUrl, function(data) {
		...
		var dataAmount = filteredData.length;

		//register aircraft texture
		/*if (!update) {
			scene += '<a-assets><img id="aircraft_texture" src="boeing.png"></a-assets>';
		}*/
...
function doRequests(autosync = false, interval) {
	loadAirplaneData(countrySelection, 1000, false);

	if (autosync) {
		var updateInt = setInterval(function() {
			console.log('update');
			loadAirplaneData(countrySelection, 1000, true);
		}, interval);
	}
}
